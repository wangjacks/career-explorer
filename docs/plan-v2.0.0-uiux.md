...
6. **表单流程情绪化**（/form/*）：步骤进度可视化、标签选择反馈微交互、提交成功页深绿仪式感 hero
7. **学生面板**：档案卡增强，顶部深绿品牌区（头像+词云展示强化）
8. **管理端品牌化**（大胆）：分组导航激活态深绿、StatCard 顶条品牌渐变、区块标题品牌色；内容区表格/表单保持白底卡片保可读
9. **动效编排**：页面入场 + CTA hover 微交互，克制编排 + prefers-reduced-motion 已就位
